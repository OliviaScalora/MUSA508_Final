...
**City Property Data** - will be filtered to account for various facillity type - child crisis center, fire and police station, public housing, arts and education centers
**Crime Data** - split into misdemeanors and felonies
...
**Light Rail Stations** - Shapefile that indicates the lightrail stations that run through Downtown, Mesa.
...
```{r zone.point.plot, echo=FALSE, results='hide', message=FALSE, warning=FALSE,fig.width = 9.75, fig.height = 8.03, out.width= "100%"}
...
```
 <br>
figure 2. illustrates the relationship of zoning districts to the opioid overdose incident points. Because the overdose incident points are rounded to the approximate 1/3 mi. interval for privacy reasons, we have aggregated the amount of overdoses at each point and visualized each point as a graduated symbol representing the count number. This is how overdose points will be illustrated throughout most the analysis.
 <br>
A spatial boundary, be it a Census block groups, or a police district, is often drawn arbitrarily or does not represent a consistent division of space. We are hoping to analyze the risk of opioid overdose evenly across space and cannot be limited by these boundaries. For this reason, we create a 'fishnet', or grid of cells at 1/3 mi dimensions, to which we will aggregate our point data and the risk predictions will be represented as a smooth surface. Figure 3 illustrates opioid overdose incidents from 2017, where darker pixels indicate a higher count of incidents.
 <br>
...
do.call(grid.arrange,c(zone_mapList, ncol=2, top="Density of Zone Type by Fishnet", bottom= "figure 4.1"))
...
<div class="plot">
```{r crime.plot, echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.width =7, fig.height = 5}

do.call(grid.arrange,c(crime_mapList, ncol=2, top="CrimeRisk Factor by Fishnet", bottom= "figure 4.2"))
...
do.call(grid.arrange,c(point_mapList, ncol=2, top="Risk Factor by Fishnet", bottom= "figure 4.3"))
...
<div class="plot">
```{r census.plot, echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.width =7, fig.height = 10}

do.call(grid.arrange,c(census_mapList, ncol=2, top="Census Variable by Fishnet", bottom= "figure 4.4"))
...
Figures 4.5 and 4.6 are a representation of the nearest neighbor function used on our City property and census data.  Tthe nearest neighbor function calculates the average distance of each fishnet cell to the closest 3 points of the variable. this creates a gradiated map because the value of each fishnet cell takes on the calculated average distance. We will determine whether we will use the unaggragated or nearset neighbor version of these variables for the final poisson regression through analysing the correlation of each independent variable to the dependent variable later in the analysis. Our next step is to explore the spatial process of opioid overdoses in Mesa.  

<br>
<div class="plot">
```{r point.nn.plot, echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.width =7, fig.height = 15}
do.call(grid.arrange,c(point_mapList.nn, ncol=2, top="Nearest Neighbor Risk Factors by Fishnet", bottom= "figure 4.5"))
```
</div>
<br>

<div class="plot">
```{r census.nn.plot, echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.width =7, fig.height = 10}
do.call(grid.arrange,c(census_mapList.nn, ncol=2, top="Census Variable Nearest Neighbor by Fishnet", bottom= "figure 4.6"))
```
</div>
<br>
...
To test the spatialo process, we us a statistic called Local Moran’s I. Here, the null hypothesis is that the overdose count at a given location is randomly distributed relative to its immediate neighbors. We create a neighbor list and a spatial weights index using **queen contiguity.**  Our output, figure 5.1, depicts where there is relatively high local clustering of overdose incidents. 


<div class="plot">
```{r local.moran.plot, echo=FALSE, message=FALSE, warning=FALSE,fig.width =10, fig.height =10}

do.call(grid.arrange,c(varList, ncol = 2, top = "Local Morans I statistics, Mesa, AZ Overdose Incidents", bottom = "figure 5.1"))
